...
def trajectories():        
    for i_episode in range(K):
        ...
        while  done==0 : #runs unless done = 1
            ...
            # Intelligent agent
            f_action , theta_action  = intelligent_agent(action_space_f , car_centre , p ,q, theta_lane)
...
if train_network == 1 : 
    ...
    for t in range(N):          
        ...
        if (t+1)%100 == 0 :
            ...
            df.to_csv('output_plots/Custom_CNN/CSV/intelligent_agent_ex1.csv')
...
cwd = os.getcwd()
output_file_name = os.path.join(cwd,"ex1_output_intelligent.mp4")
