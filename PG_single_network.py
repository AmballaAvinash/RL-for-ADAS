...
from algorithm import PolicyNetworkSN
...
def trajectories():        
    ...
    for i_episode in range(K):
        ...
        while  done==0 : #runs unless done = 1
            ...
            # PG 
            action , prob = net.select_action(state)
            ...
            P.append(prob) # Need to look into this
            #print(reward)
            g.append(reward)
...
net = PolicyNetworkSN(len(action_space_f) , len(action_space_theta) , image_shape , len(action_space)) 
...
if train_network == 1 : 
    ...
    for t in range(N):
        ...
        if (t+1)%100 == 0:
            ...
            plt.xlabel('Episode')
            plt.ylabel('Reward')           
            plt.savefig('output_plots/Custom_CNN/test91.png')
            plt.show()
            ...
            plt.xlabel('Episode')
            plt.ylabel('Loss')            
            plt.savefig('output_plots/Custom_CNN/loss91.png')
            plt.show()
            ...
            df.to_csv('output_plots/Custom_CNN/CSV/PG_single_network_ex1.csv')
    ...
    #loading the model
    net = torch.load("saved_model_PG_SN.pt")
...
cwd = os.getcwd()
output_file_name = os.path.join(cwd,"ex1_output_PG_SN.mp4")
